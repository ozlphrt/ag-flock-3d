...
export function createGPGPUSimulation(renderer: THREE.WebGLRenderer, population: number, state?: SimulationState): GPGPUController {
    ...
    return {
        ...
        update: (time: number, delta: number, state: SimulationState) => {
            ...
            // Silky Serene Cruising & Ultra-Smooth Morphing Dynamics (Fast for initial startup emergence <=3.5s, graceful slow-burn for continuous transitions)
            ...
            const activeLerpRate = (isFastMorph ? 0.22 : (isMorphing ? 0.040 : 0.065)) * speedScale;
            const activeMaxSpeed = (isFastMorph ? 0.55 : (isMorphing ? 0.16 : 0.20)) * speedScale;
            const activeMaxAccel = (isFastMorph ? 0.045 : (isMorphing ? 0.008 : 0.012)) * speedScale;
