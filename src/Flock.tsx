...
export function Flock({ count, state, setPopulation }: FlockProps) {
    ...
    useFrame((stateContext, delta) => {
        ...
        // 🌪️ Local 3D Tornado Cyclone Field (TIGHTLY LOCALIZED TWISTERS WITH HIGH-VELOCITY CROWN SLINGSHOTS)
        const tornadoCount = state.tornadoCount !== undefined ? state.tornadoCount : 2;
        const tornadoStrength = (state.tornadoStrength ?? 1.5) * (state.speedMultiplier || 1.0);
        const tornadoCentrifugal = (state.tornadoCentrifugal ?? 1.8);
        ...
        if (tornadoCount > 0) {
            // Cyclone 1: Wandering elliptical twister with dynamic precession tilt
            if (tornadoCount >= 1) {
                const a0 = time * 0.28;
                const bx0 = fastCos(a0) * 6.5;
                const bz0 = fastSin(a0) * 5.5;
                const by0 = -7.0 + fastSin(time * 0.45) * 1.5;

                const tx0 = bx0 + fastCos(a0 + 0.8) * 1.8;
                const tz0 = bz0 + fastSin(a0 + 0.8) * 1.8;
                const ty0 = by0 + 13.5;
                ...
                tActive.push({
                    ...
                    H: H0, invH: invH0,
                    neckR: 0.45, crownR: 2.2,
                    infRSq: 6.25, // Tightly localized: R = 2.5 (leaves 90% of topology 100% pristine)
                    swirlDir: 1.0
                });
            }
            ...
            if (tornadoCount >= 2) {
                const a1 = time * -0.22 + Math.PI;
                const bx1 = fastCos(a1) * 7.2;
                const bz1 = fastSin(a1 * 1.3) * 6.0;
                const by1 = -6.8 + fastCos(time * 0.4) * 1.6;

                const tx1 = bx1 + fastSin(a1) * 2.0;
                const tz1 = bz1 + fastCos(a1) * 2.0;
                const ty1 = by1 + 13.8;
                ...
                tActive.push({
                    ...
                    H: H1, invH: invH1,
                    neckR: 0.50, crownR: 2.4,
                    infRSq: 7.0, // Tightly localized: R = 2.65
                    swirlDir: -1.0
                });
            }
            ...
            if (tornadoCount >= 3) {
                const a2 = time * 0.18 + 2.2;
                const bx2 = fastSin(a2 * 1.2) * 5.5;
                const bz2 = fastCos(a2) * 7.0;
                const by2 = -7.0 + fastSin(time * 0.35) * 1.8;

                const tx2 = bx2 + fastCos(a2 * 0.8) * 2.2;
                const tz2 = bz2 + fastSin(a2 * 0.8) * 2.2;
                const ty2 = by2 + 13.2;
                ...
                tActive.push({
                    ...
                    H: H2, invH: invH2,
                    neckR: 0.40, crownR: 2.0,
                    infRSq: 5.76, // Tightly localized: R = 2.4
                    swirlDir: 1.0
                });
            }
        }

        // Update visual tornado meshes
        ...
        for (let tIdx = 0; tIdx < 3; tIdx++) {
            ...
            if (tIdx < tActive.length) {
                ...
                m.scale.set(1, T.H / 14.0, 1);
                m.rotation.y += 0.08 * T.swirlDir;
            } else {
                m.visible = false;
            }
        }
        ...
        for (let i = 0; i < boidCount; i++) {
            ...
            for (let tIdx = 0; tIdx < tLen; tIdx++) {
                ...
                if (hNorm >= -0.05 && hNorm <= 1.25) {
                    ...
                    if (rSq < T.infRSq) {
                        ...
                        if (hNorm > 0.68) {
                            const crownProg = (hNorm - 0.68) / 0.36;
                            const slingshotMag = crownProg * crownProg * 0.42 * tornadoCentrifugal;
                            velX[i] += (rxNorm * 1.5 + txNorm * 1.0) * slingshotMag;
                            velY[i] += (ryNorm * 0.4 + T.uy * 0.8) * slingshotMag;
                            velZ[i] += (rzNorm * 1.5 + tzNorm * 1.0) * slingshotMag;
                            inTornadoCrown = true;
                        }
                    }
                }
            ...
            const currentMaxDisp = inTornadoCrown ? (activeMaxDisp * 3.5) : activeMaxDisp;
    ...
    return (
        <group>
            ...
            {/* Ethereal Luminous Tornado Funnel Indicators */}
            <mesh ref={tornadoMesh0} visible={false}>
                <cylinderGeometry args={[2.2, 0.45, 14.0, 16, 8, true]} />
                <meshBasicMaterial
                    color="#00ffcc"
                    wireframe={true}
                    transparent={true}
            ...
            <mesh ref={tornadoMesh1} visible={false}>
                <cylinderGeometry args={[2.4, 0.50, 14.0, 16, 8, true]} />
                <meshBasicMaterial
                    color="#ffaa00"
                    wireframe={true}
                    transparent={true}
            ...
            <mesh ref={tornadoMesh2} visible={false}>
                <cylinderGeometry args={[2.0, 0.40, 13.5, 16, 8, true]} />
                <meshBasicMaterial
                    color="#ff00aa"
                    wireframe={true}
                    transparent={true}
